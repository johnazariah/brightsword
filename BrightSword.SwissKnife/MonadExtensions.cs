...
{
	public static TResult Maybe<T, TResult>(this T _this, Func<T, TResult> func, TResult defaultResult = default(TResult)!) where T : class
	{
	...
	{
		return _this.Maybe((T _) => predicate(_) ? func(_) : defaultResult, defaultResult);
	}

	public static T When<T>(this T _this, Func<T, bool> predicate, Action<T> action) where T : class
	{
		return _this.Maybe(delegate(T _)
		{
			if (predicate(_))
			{
				action(_);
